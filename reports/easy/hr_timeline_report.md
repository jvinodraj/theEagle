...
- Efficiency Factor (EF, W/bpm — Joe Friel / TrainingPeaks standard): first run 1.536 W/bpm -> last run 1.625 W/bpm (+5.8%). Reference: <1.4 beginner, 1.4–1.8 recreational, 1.8+ trained.
- Aerobic Decoupling (Garmin / Joe Friel standard — <5% = aerobically fit run): median 7.1%, 3/19 runs below the 5% threshold. This metric is load-independent — valid for comparing 5 km and 10 km easy runs.
- Latest run score 54.0/100 (EF 65.9, decoupling 34.1, stability 61.0).
...
- Steady-state easy pace changed +2.3% from the first comparable run to the latest run.
- Steady power-per-heartbeat changed +5.8% across the timeline, which is the cleanest aerobic-efficiency signal in this dataset.
- Aerobic decoupling moved +1.0 percentage points from the opening run to the latest run.
- 16 runs were recorded at 30 C or hotter; heat is a plausible confounder for drift and HR cost in this sample.
- 6/19 runs crossed the 8% decoupling line, which is the strongest within-run fatigue signal in these easy sessions.
